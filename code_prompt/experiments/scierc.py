...
from ..prompts.scierc import get_nl_prompt, get_code_prompt
...
def run_scierc(
    ...
    for example in tqdm(testset):
        if "code" in model_name.lower():
            ...
            if enforce_code_prompt:
                ...
                prompt = get_nl_prompt(example, few_shot_examples=few_shot_examples)
                response = get_response(
                    prompt,
                    client=model,
                    model=model_name,
    ...
    logging.info(f"SciERC results saved to {output_path}")

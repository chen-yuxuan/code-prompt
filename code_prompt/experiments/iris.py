...
from ..prompts.iris import get_nl_prompt, get_code_prompt, relabel_iris
...
def run_iris(
    ...
    for example in tqdm(testset):
        ...
        examples.append(
            {
                "id": example["Id"],
                "SepalLengthCm": example["SepalLengthCm"],
                "SepalWidthCm": example["SepalWidthCm"],
                "PetalLengthCm": example["PetalLengthCm"],
                "PetalWidthCm": example["PetalWidthCm"],
    ...
    if shots == 0:
        output_path = f"./outputs/iris_{_model_name}_{shots}_shot"
    else:
        output_path = f"./outputs/iris_{_model_name}_{shots}_shot_seed_{seed}"
    ...
    logging.info(f"Iris results saved to {output_path}")
